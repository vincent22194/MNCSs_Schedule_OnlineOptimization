...
for t_start = 1 : Env.Ntotal : Total_Sim_Slots
    ...
    for p_idx = 1 : size(P_perf_perms, 1)
        ...
        for i = Control_Idx
            ...
            for k_step = 1 : length(sv_seq_pred{i})
                ...
                % if dk == Env.T_i(i)+1
                %     comm_penalty = 50 * Lambda_delay * dk * Route_hop(i);
                % else
                %     comm_penalty = Lambda_delay * dk * Route_hop(i);
                %     % 对丢包惩罚大
                % end

                comm_penalty = Lambda_delay * dk * Route_hop(i);

                current_J_total = current_J_total + Env.T_i(i) * (x_temp' * Q_weight * x_temp) +  comm_penalty;
    ...
    for i = Control_Idx
        ...
        for k_r = 1 : length(sv_seq_real{i})
            ...
            comm_penalty = Lambda_delay * dk_r * Route_hop(i);
...
for i = Control_Start_Idx : Control_End_Idx
    ...
    title(['Loop ', num2str(i-Non_control_N), ' (T_i = ', num2str(Env.T_i(i)*0.01), ' s)']);
    xlabel('时间 (Seconds)');
    ylabel('状态偏差 x');
...
% 
%     title(['Loop ', num2str(i-Non_control_N)]);
%     xlabel('时延 (Slots)');
%     ylabel('频次');
%     xlim([0, max_delay + 1]);
% end
...
% imagesc(Data_Plot);
% 
% xt = 0.5 : 1 : num_cols + 1.5;
% xticks(xt);
% xticklabels(compose('%.2f', (0:num_cols-1)*Env.Ntotal * 0.01));
% 
% % 绘制垂直分割线
% for x = 0.5 : 1 : num_cols + 0.5
%     xline(x, 'Color', 'w', 'LineWidth', 1);
% end
% 
% % 绘制水平分割线
...
%     'TickLength', [0 0]); 
% ylabel('Loops', 'FontSize', 10);
% xlabel('时间(s)', 'FontSize', 10);
% title('优先级演变', 'FontSize', 10);
% axis tight; 
% box on;
